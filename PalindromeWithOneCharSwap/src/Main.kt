...
fun findPal(cur: String, start: Int, end: Int, swapped: Boolean): String {
	...
	if(text[start] == text[end])
		newPal = findPal(text[start] + cur + text[end], start - 1, end + 1, swapped)         // (Step 3.2)
	else if(!swapped)
		newPal = findPal(text[start] + cur + text[end], start - 1, end + 1, !swapped)
	else
		return cur
	return newPal
}
...
fun findLongest() {
	...
	for(i in 1..text.length) {
		if(i + 1 < text.length){                                  //  If the palindrom's middle is 1 char
			var attempt = findPal("" + text[i], i - 1, i + 1, false)           // (Step 1)
		...
		if(i <= text.length - 2) {              //  If the palindrom to be's middle is 2 chars
			if(text[i] == text[i + 1]) {        //  Then it checks the word             (Step 2.1 and 2.2)
				var attempt = findPal("" + text[i] + text[i + 1], i - 1, i + 2, false)
				if(attempt.length > length) {                   // Step 4
					length = attempt.length
					longestPal = attempt
				}
